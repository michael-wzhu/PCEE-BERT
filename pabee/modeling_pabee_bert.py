...
class BertModelWithPabeeV0(BertModel):
    """
...
class BertModelWithPabeeV1(BertModel):
    """
    ...
    def log_stats(self, to_dir=None):
        avg_inf_layers = self.inference_layers_num / self.inference_instances_num
        message = f"*** Patience = {self.patience} Avg. Inference Layers = {avg_inf_layers:.5f} Speed Up = {1 - avg_inf_layers / self.config.num_hidden_layers:.5f} ***"
        print(message)
    ...
    def forward(
        ...
        if self.training or self.patience == -1:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                if cur_entropy < exiting_threshold:
                    patient_counter += 1
                else:
                    patient_counter = 0
                ...
                all_prev_logits.append(cur_probs)

                if patient_counter == 0:
                    all_prev_logits_close = [cur_probs, ]
                else:
                    all_prev_logits_close.append(cur_probs)

                if patient_counter == self.patience:
                    break

            print("all_prev_logits_close: ", len(all_prev_logits_close))

            if do_ensemble == 0:
                res = [cur_logits, ]
            elif do_ensemble == 1:
                res = [torch.mean(torch.stack(all_prev_logits_close, dim=0), dim=0, keepdim=False)]
            else:
                res = [torch.mean(torch.stack(all_prev_logits, dim=0), dim=0, keepdim=False)]
...
class BertForSequenceClassificationWithPabee(BertPreTrainedModel):
    def __init__(self, config,):
        ...
        if config.ee_mechanism == "V0":
            self.bert = BertModelWithPabeeV0(config)
        elif config.ee_mechanism == "V1":
            self.bert = BertModelWithPabeeV1(config)
        else:
            raise ValueError(
